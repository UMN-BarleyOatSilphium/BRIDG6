...
GWAS = read.csv("~/Dropbox/SmithLab/NAM/Analysis/GWAS/input/genos_LDKNNI.csv", check.names=F,row.names=1)
...
#parents<-gen[,1:92]
parents<-as.data.frame(t(GWAS)) # all samples
...
print (paste("SNP removed for MAF <3% = ", length(which(mafparents <0.03)) ,sep=" ")) #2599 BRIDG6
...
for (n in 1:7){
  ...
  write.table(MAPfile,paste("~/Dropbox/SmithLab/NAM/Analysis/LD_BRIDG6parents/Plink_bridg6/CHR_",n,".map",sep=""), quote=F,row.names=F,col.names=F,sep="\t")
  ...
  write.table(PEDfile,paste("~/Dropbox/SmithLab/NAM/Analysis/LD_BRIDG6parents/Plink_bridg6/CHR_",n,".ped",sep=""), quote=F,row.names=F,col.names=F,sep="\t")
...
for (chr in 1:7){
  ...
  output<-read.table(paste("~/Dropbox/SmithLab/NAM/Analysis/LD_BRIDG6parents/Plink_bridg6/Output_maf3/snp-thin_chr",chr,".ld",sep=""),header=T)
...
pdf("/home/agonzale/Dropbox/SmithLab/NAM/Analysis/LD_BRIDG6parents/Plink_bridg6/Plots/LD_MAF3/LD_bridgALL_noThin_snpGWAS_maf3.pdf",width=7,height=5)
par(mar=c(4,5,1,4))
...
##############################################################################################
##############3 PLOT LD PER CHROMOSOME
#############################################################################################
LDresults<-data.frame()
for (chr in 1:7){
  #output<-read.table(paste("/Users/Mia/Dropbox/SmithLab/NAM/Analysis/LD_BRIDG6parents/r2_plink/snp-thin_chr",chr,".ld",sep=""),header=T)
  output<-read.table(paste("~/Dropbox/SmithLab/NAM/Analysis/LD_BRIDG6parents/Plink_bridg6/Output/snp-thin_chr",chr,".ld",sep=""),header=T)
...
#pdf("~/Desktop/LD_bridgParents.pdf",width=7,height=5)
pdf(paste("~/Dropbox/SmithLab/NAM/Analysis/LD_BRIDG6parents/Plink_bridg6/Plots/PetChromosome/LD_chr",chr,".pdf",sep=""),width=7,height=5)

plot(LDresults[,1]/1000000,LDresults[,2],cex=0.2,col="light gray",xlab="Distance (Mbp)",ylab="r^2", main=paste("Chr",chr,sep=""))
...
###################################################################################################################
######################### Calculate average R2 at different bp distances
#####################################################################################################################
...
less5mpb<-LDresults[which(LDresults[,1] <= 5000000),]
...
AvePoints<-round(max(SortedResults_Mbp[,1])/end)
Average<-matrix(NA,ncol=4,nrow=AvePoints)
colnames(Average)<-c("start(Mbp)","end(Mbp)","Mean","Median")

for (i in 1:AvePoints){
  aveWindows<-mean(SortedResults_Mbp[which(as.numeric(SortedResults_Mbp[,1]) >= start & as.numeric(SortedResults_Mbp[,1]) <= end),2])
  medianWindows<-median(SortedResults_Mbp[which(as.numeric(SortedResults_Mbp[,1]) >= start & as.numeric(SortedResults_Mbp[,1]) <= end),2])
  
  Average[i,1]<-start
  Average[i,2]<-end
  Average[i,3]<-aveWindows
  Average[i,4]<-medianWindows
  
  start<-end+1
  end<-start + lengthwindows
}

write.table(Average,"~/Dropbox/SmithLab/NAM/Analysis/LD_BRIDG6parents/Plink_bridg6/Plots/Table_aveLD_distance.txt",quote=F,row.names=F,col.names=T,sep="\t")
