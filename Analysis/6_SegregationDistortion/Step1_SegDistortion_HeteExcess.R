...
for (i in 1:(dim(FAMILY)[1])){
  ...
  Pval_out<-as.data.frame(cbind(as.character(FREQ[,1]),P_value_out))
  colnames(Pval_out)<-c("SNP",as.character(FAMILY[i,1]))
  assign(paste("AA_FREQ_",i, sep=""), FREQ)
  assign(paste("AA_pval_",i, sep=""), Pval_out)
  ...
  plot(genotype_counts_pvalue$Index,genotype_counts_pvalue[,as.character(FAMILY[i,1])], ylab="Frequency Rasmusson genotype",xlab="SNP sorted",cex=0.4, main=as.character(FAMILY[i,1]))
...
write.table(MY_TABLE_or_pvalue, "~/Documents/SmithLab/NAM/Analysis/SNPdensity_and_segDist/SegDist_091817/Output/88Fam_genmap_SNPdonorgenotypeFreqAA_pval.txt", quote=F,row.names=F,col.names=T,sep="\t")
