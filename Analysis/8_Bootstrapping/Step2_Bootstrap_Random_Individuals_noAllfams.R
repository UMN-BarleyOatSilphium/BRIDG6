...
# Sampling random number of individuals from each family (264,528,968,2012)
##########################################################################################################################
rm(list=ls())
...
write.table(as.data.frame(RESULTS_SIGNIF), paste("/home/smithkp/agonzale/Projects/NAM/Analysis/GWAS_bootstrap/OUTPUT_randomNoFam/Random_",TOTALrandomSamples,"/Number_sig_SNPs_90to100.txt",sep=""),quote=F,row.names=F,col.names=F,sep="\t")
# The number of times that each SNP was found significant 
write.table(as.data.frame(table(SNPs_significant)), paste("/home/smithkp/agonzale/Projects/NAM/Analysis/GWAS_bootstrap/OUTPUT_randomNoFam/Random_",TOTALrandomSamples,"/Count_sig_per_SNP_90to100.txt",sep=""),quote=F,row.names=F,col.names=F,sep="\t")
}