# Author: Ana Poets
# Description: This code takes the HAPMAP file after basic vcf filtering , leaving SNPs with upto 80% missing, devides it in families. Adding a consensus copy of Rasmusson.  
# It selects only those SNPs that are present in Rasmusson and the Donor parent
#########################################################################################################################################
...
#HAPMAP AFTER converting homo to hete missing
HAPMAP <-read.table("/home/smithkp/agonzale/Shared/GBS_Projects/NAM_GBS_2_6row/TASSEL_072016/Filter/NAM_MNS_July2016_DP5_GQ30_mis80_5775ind.recode_transformedHETE_toNA_HH_hmp.txt",header=T,row.names=1)
...
for (i in 1:(dim(Families)[1])){
	if (Families[i,2] != "HR655S"){
		...
		write.table(HAPMAP_family, paste("/home/smithkp/agonzale/Shared/GBS_Projects/NAM_GBS_2_6row/TASSEL_072016/Analysis/Hapmap_mis80/",Families[i,2],"_shrSNP_hmp.txt",sep =""), quote=F, row.names=T, col.names=T, sep="\t")
	}	
	if (Families[i,2] == "HR655S"){
		...
		write.table(HAPMAP_family_HR655, paste("/home/smithkp/agonzale/Shared/GBS_Projects/NAM_GBS_2_6row/TASSEL_072016/Analysis/Hapmap_mis80/",Families[i,2],"_shrSNP_hmp.txt",sep =""), quote=F, row.names=T, col.names=T, sep="\t")
	}
